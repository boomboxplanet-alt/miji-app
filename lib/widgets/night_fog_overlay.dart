...
class NightFogOverlay extends StatelessWidget {
  const NightFogOverlay({super.key, this.opacity = 0.18, this.starDensity = 0.8});
...
class _NightFogPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final glowPaint = Paint()
      ...
      ..color = const Color(0xFFA58BD9).withOpacity(0.10);
    canvas.drawCircle(Offset(size.width * 0.25, size.height * 0.3), size.shortestSide * 0.28, glowPaint);
    canvas.drawCircle(Offset(size.width * 0.8, size.height * 0.7), size.shortestSide * 0.22, glowPaint..color = const Color(0xFFF0D9C8).withOpacity(0.10));
    ...
    for (int i = 0; i < starCount; i++) {
      ...
      final color = const Color(0xFFF7F2E9).withOpacity(alpha * opacity);
  ...
  bool shouldRepaint(covariant _NightFogPainter oldDelegate) {
    return oldDelegate.opacity != opacity || oldDelegate.starDensity != starDensity;
  }
}
