...
class MessageProvider extends ChangeNotifier {
  ...
  Future<void> generateLocalizedBotMessages() async {
    ...
    try {
      List<Message> botMessages =
          await _botMessageService.generateLocalizedBotMessages(
        _currentLatitude!,
        _currentLongitude!,
      );
  ...
  Future<void> generateFirstLaunchBotMessages() async {
    ...
    try {
      List<Message> botMessages =
          await _botMessageService.generateFirstLaunchBotMessages(
        _currentLatitude!,
        _currentLongitude!,
      );
  ...
  Future<void> generateThematicBotMessages(String theme) async {
    ...
    try {
      List<Message> botMessages =
          await _botMessageService.generateThematicBotMessages(
        _currentLatitude!,
        _currentLongitude!,
        theme,
      );
  ...
  void updateUserLocation(double latitude, double longitude, {double? radius}) {
    ...
    final realUserMessages = _messages
        .where((message) =>
  ...
  Future<Message> sendMessage({
    ...
    try {
      ...
      } else if (customSenderName != null &&
          customSenderName.trim().isNotEmpty) {
  ...
  // 機器人相關方法
  void addBotMessage(String content, double lat, double lng, double radius,
      Duration duration) {
  ...
  void _generateBotMessageStats(String messageId) {
    ...
    Timer(Duration(seconds: random.nextInt(5) + 1), () {
      ...
      if (messageIndex != -1) {
        ...
        final initialLikes =
            random.nextInt(initialViews ~/ 2 + 1); // 點讚數不超過觀看數的一半
